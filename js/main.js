...
const getRandomLengthMixedArray = function(croppedArray) {
  const randomNumberFirst = getRandomNumber(0,croppedArray.length-1);
  const randomNumberSecond = getRandomNumber(0,croppedArray.length-1);
  const newMixArray = mixArray(croppedArray);
  return (randomNumberFirst > randomNumberSecond) ? newMixArray.slice(randomNumberSecond, randomNumberFirst) : newMixArray.slice(randomNumberFirst, randomNumberSecond);
};
...
const createOfferRentRealty = function(userNumber) {
  ...
  const getAvatarValue = function() {
    return userNumber < 10 ? `img/avatars/user${  0  }${userNumber}.png`: `img/avatars/user${userNumber}.png`;
  };
...
let numberAvatars = new Array(10).fill(null).map((value,index) => value += index + 1);
